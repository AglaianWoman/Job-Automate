...
from jobautomate import application
...
def cli(debug, key, xvfb, verbose, first_name, last_name, email_address,
    ...
    user_parameters = application.indeed_parameters(job_description, job_location)
    response = application.access_indeed_api(user_parameters, key)
    ...
    while True:
        for url in application.retrieve_indeed_urls(response):
            driver.get(url)
            try:
                application.open_application(driver, 'indeed-apply-button')
                application.switch_frames(driver, 'iframe[name$=modal-iframe]')
                application.fill_application(driver, first_name, last_name, email_address, resume)
                if debug:
                    application.submit_application(driver, debug=True)
                else:
                    application.submit_application(driver)
